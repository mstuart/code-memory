use code_memory::approval::workflow::{ApprovalWorkflow, PendingKnowledge, ApprovalStatus};

#[tokio::test]
async fn test_submit_for_approval() {
    let workflow = ApprovalWorkflow::new("postgresql://localhost/test_db").await.unwrap();
...
async fn test_reject_knowledge() {
    ...
    let id = workflow.submit(pending).await.unwrap();
    workflow.reject(&id, "reviewer_789", "Contains secrets").await.unwrap();
